...
+ (void)postModifyTheUserDataInterfaceRequestWithUserId:(NSString *)userId userName:(NSString *)userName userText:(NSString *)userText  companyPosition:(NSString *)companyPosition companyName:(NSString *)companyName Completion:(requestComplete)completion{
    NSDictionary *parameters = @{@"userId": userId ,@"userName":userName ,@"userText":userText ,@"companyPosition": companyPosition,@"companyName": companyName};
//    NSLog(@"%@",parameters );
    AFHTTPSessionManager *session = [AFHTTPSessionManager manager];
    session.requestSerializer = [AFHTTPRequestSerializer serializer];
    session.responseSerializer = [AFHTTPResponseSerializer serializer];
