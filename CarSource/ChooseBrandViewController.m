...
- (void)viewDidLoad {
    ...
    _brandTable = [[UITableView alloc]initWithFrame:CGRectMake(0, 114, SCREEN_WIDTH, SCREEN_HEIGHT -114) style:UITableViewStylePlain];
...
//    _isChoose[indexPath.section][indexPath.row] = !_isChoose[indexPath.section][indexPath.row];
    ...
    if (self.chooseBrand == 12) {
        ...
        self.vehicleId([NSString stringWithFormat:@"%@",model.brandId],[NSString stringWithFormat:@"%@",model.brandName],_chooseType);
        [self.navigationController popViewControllerAnimated:YES];
    }else{
        _integer = 0;
