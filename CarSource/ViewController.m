    ...
    self.mall.brandTag = ^(NSInteger brandTag){
        ...
        switch (brandTag) {
            case 10:
            {
                ...
                chooseBrand.vehicleId = ^(NSString *vehicleId,NSString *vehicleName,NSString *specificationId){
...
//        [self.navigationController pushViewController:nextView animated: YES];
    ...
    _viewControllers = [[NSArray alloc]initWithObjects:home,self.mall,community,myProfile, nil];
...
//     NSArray *tabBarArray = @[@"首页",@"资源",@"消息",@"我的"];
    for (int i = 0; i < count; i++)
    {
        ...
        button.frame = CGRectMake((SCREEN_WIDTH/4)*i +((SCREEN_WIDTH/4)/2-20), 2, 40 , 45);
        button.tag = i;
...
//        [button setTitleColor:[UIColor colorWithRed:18/255.0 green:152/255.0 blue:234/255.0 alpha:1]forState:UIControlStateSelected];
        [button setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"flat%d",i+1]] forState:UIControlStateNormal];
        [button setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"sky%d",i+1]] forState:UIControlStateSelected];
